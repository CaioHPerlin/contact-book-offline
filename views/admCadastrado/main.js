...
api.on('user-getall-res', (_, { success, data }) => {
	...
	data.map((user) => {
		const userElement = `<li class="list-group-item d-flex align-items-center">
...
api.on('user-delete-res', (_, { success }) => {
	if (success) api.send('user-getall-req', searchBox.value);
});
